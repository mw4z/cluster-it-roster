...
// API: Get roster for a specific date (defaults to today)
app.get('/api/roster', (req, res) => {
  try {
    ...
    // Always compute available dates
    ...
    for (let i = employeeStartRow; i <= Math.min(employeeEndRow, rawData.length - 1); i++) {
      ...
      shifts[shift.label].push({
        name,
        calledIn: shift.calledIn,
        shiftCode: shift.code,
        rawTime: shift.rawTime
      });
    }
    ...
    for (const label of shiftOrder) {
      if (shifts[label]) {
        sortedShifts.push({ label, employees: shifts[label] });
      }
    }
